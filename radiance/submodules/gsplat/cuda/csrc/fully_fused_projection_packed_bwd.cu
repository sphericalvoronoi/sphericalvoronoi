...
__global__ void fully_fused_projection_packed_bwd_kernel(
    ...
    const int64_t *__restrict__ primitive_ids, // [nnz]
    ...
    const int64_t gid = primitive_ids[idx]; // gaussian id
...
fully_fused_projection_packed_bwd_tensor(
    ...
    const torch::Tensor &primitive_ids,                // [nnz]
    ...
    GSPLAT_CHECK_INPUT(primitive_ids);
        ...
        fully_fused_projection_packed_bwd_kernel<float>
            <<<(nnz + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS,
               ...
               stream>>>(
                ...
                primitive_ids.data_ptr<int64_t>(),

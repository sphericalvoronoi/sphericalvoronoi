...
__global__ void isect_tiles(
    ...
    const int64_t *__restrict__ primitive_ids, // [nnz] optional
    // data
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> isect_tiles_tensor(
    ...
    const at::optional<torch::Tensor> &primitive_ids, // [nnz]
    const uint32_t C,
    const uint32_t tile_size,
    const uint32_t tile_width,
    const uint32_t tile_height,
    ...
    int64_t *primitive_ids_ptr = nullptr;
    if (packed) {
        nnz = means2d.size(0);
        total_elems = nnz;
        TORCH_CHECK(
            camera_ids.has_value() && primitive_ids.has_value(),
            "When packed is set, camera_ids and primitive_ids must be provided."
        );
        camera_ids_ptr = camera_ids.value().data_ptr<int64_t>();
        primitive_ids_ptr = primitive_ids.value().data_ptr<int64_t>();
    } else {

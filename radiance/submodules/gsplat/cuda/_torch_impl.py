...
def accumulate(
    ...
    {primitive_ids, pixel_ids, camera_ids}, which annotates the intersection between all
    ...
    Args:
        ...
        primitive_ids: Collection of Gaussian indices to be rasterized. A flattened list of shape [M].
    ...
    deltas = pixel_coords - means2d[camera_ids, primitive_ids]  # [M, 2]
    c = conics[camera_ids, primitive_ids]  # [M, 3]
    sigmas = (c[:, 0] * deltas[:, 0] ** 2 + c[:, 2] * deltas[:, 1] ** 2) + 2 * c[
        :, 1
    ] * deltas[:, 0] * deltas[
        :, 1
    ]  # [M]
    alphas = torch.clamp_max(
        opacities[camera_ids, primitive_ids]
        * torch.pow(1.0 - sigmas, betas[camera_ids, primitive_ids]),
        0.999,
    )
    ...
    renders = accumulate_along_rays(
        weights,
        colors[camera_ids, primitive_ids],
        ray_indices=indices,
        n_rays=total_pixels,

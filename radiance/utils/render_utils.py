...
def create_videos(base_dir, input_dir, out_name, num_frames=480, fps=24, repeat=2):
    ...
    first_img_path = os.path.join(input_dir, 'renders', f'{idx_to_str(0)}.png')
    if not os.path.exists(first_img_path):
        print(f"File immagine iniziale non trovato: {first_img_path}")
        return
    ...
    try:
        for r in range(repeat):  # 👈 ripete il video
            print(f"Pass {r+1}/{repeat}")
            for idx in range(num_frames):
                img_file = os.path.join(input_dir, 'renders', f'{idx_to_str(idx)}.png')
                if not os.path.exists(img_file):
                    raise ValueError(f'Image file {img_file} does not exist.')

                img = load_img(img_file) / 255.

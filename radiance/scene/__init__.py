...
class Scene:
    ...
    def eval(self, name_set, dump_images=False, eval_lpips=False, iteration=None):
        ...
        with torch.no_grad():
            for v in test_view_stack[:min(3, len(test_view_stack))]:
                _ = self.beta_model.render(v)

        for viewpoint in test_view_stack:
            _ = self.beta_model.render(viewpoint)

        for idx, viewpoint in tqdm(enumerate(test_view_stack)):
            render_pkg = self.beta_model.render(viewpoint)
        ...
        result = psnr_test, ssim_test, lpips_test
